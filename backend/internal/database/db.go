...
func createTables() error {
	...
	// 발주 예측 설정 테이블
	_, err = DB.Exec(`
		CREATE TABLE IF NOT EXISTS order_forecast_config (
			id                INTEGER PRIMARY KEY AUTOINCREMENT,
			user_id           INTEGER NOT NULL,
			vendor_item_id    INTEGER NOT NULL,
			...
			season_end_date   TEXT DEFAULT '',
			safety_stock_days INTEGER DEFAULT 3,
			buffer_ratio      REAL DEFAULT 0.15,
			is_active         INTEGER DEFAULT 1,
			created_at        DATETIME DEFAULT CURRENT_TIMESTAMP,
			updated_at        DATETIME DEFAULT CURRENT_TIMESTAMP,
			UNIQUE(user_id, vendor_item_id)
	...
	_, err = DB.Exec(`
		CREATE TABLE IF NOT EXISTS forecast_snapshot (
			...
			avg_14d                           REAL DEFAULT 0,
			avg_30d                           REAL DEFAULT 0,
			forecast_daily_sales              REAL DEFAULT 0,
			current_stock                     INTEGER DEFAULT 0,
			inbound_stock                     INTEGER DEFAULT 0,
			lead_time_days                    INTEGER DEFAULT 14,
			leadtime_demand                   REAL DEFAULT 0,
			season_end_date                   TEXT DEFAULT '',
			remaining_season_days             INTEGER DEFAULT 0,
			available_sales_days_after_inbound INTEGER DEFAULT 0,
			target_stock_days                 INTEGER DEFAULT 0,
			safety_stock_qty                  REAL DEFAULT 0,
			recommended_qty                   INTEGER DEFAULT 0,
			final_order_qty                   INTEGER DEFAULT 0,
			days_until_out                    REAL DEFAULT 0,
			alert_level                       TEXT DEFAULT 'normal',
			recommendation_status             TEXT DEFAULT 'normal',
			reason                            TEXT DEFAULT '',
			created_at                        DATETIME DEFAULT CURRENT_TIMESTAMP,
			UNIQUE(user_id, vendor_item_id, snapshot_date)
		);
	`)
	if err != nil {
		return err
